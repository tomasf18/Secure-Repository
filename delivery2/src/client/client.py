...
def saveContext(session_file, session_file_content, result):
    session_file_content["counter"] += 1                     # Update counter to the one used by the last request
    session_file_content["nonce"] = result["nonce"]          # Update nonce to the one generated by the server and received in response
...
def rep_create_org(org, username, name, email, pub_key_file):
    ...
    pub_key_str = base64.b64encode(pub_key.public_bytes(
        ...
        )).decode('utf-8') # Convert to string so that it can be sent in the request to the server (bytes are not serializable)
...
def rep_list_subjects(session_file, username=None):
    ...
    session_file_path = os.getenv("CLIENT_SESSION_FILE_PATH") + session_file + ".json"
    session_file_content = read_file(session_file_path)
    if session_file_content is None:
        logger.error(f"Error reading session file: {session_file_path}")
        sys.exit(ReturnCode.INPUT_ERROR)
    ...
    session_key = base64.b64decode(session_file_content["session_key"].encode())

    data = {
        "session_id": session_id,
        "counter": session_file_content["counter"] + 1, # Increment counter for each request
        "nonce": session_file_content["nonce"],         # TODO: DONT UPDATE NONCE
    }
    ...
    saveContext(session_file_path, session_file_content, result)
