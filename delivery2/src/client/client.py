...
def saveContext(session_file, session_context, result):
    session_context["counter"] += 1                     # Increment counter for each request
    session_context["nonce"] = result["nonce"]          # Update nonce to the one generated by the server and received in response
...
def rep_list_subjects(session_file, username=None):
    ...
    data = {
        ...
        "counter": session_context["counter"] + 1, # Increment counter for each request
